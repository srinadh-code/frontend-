...
//   ChevronDown, FileText, Download, Loader2, Github, Linkedin, GraduationCap, Link as LinkIcon
// } from 'lucide-react';
...
//     email: "kavyaachhe@gmail.com",
//     phone: "9876543210",
//     linkedin: "linkedin.com/in/srinadh",
//     github: "github.com/kavya",
//     summary: "Motivated Python Developer with experience in Django and REST APIs.",
//     skills: [
//         { category: "Languages", names: "Python, JavaScript" },
//         { category: "Frameworks", names: "Django, React" }
//     ],
//     educations: [{ degree: "B.Tech", institution: "XYZ College", period: "2022-2026" }],
//     projects: [{ title: "Resume Management System", tech: "Django, React", desc: "Built a platform to manage candidate resumes." }],
//     customSections: []
//   });

//   const [activeSections, setActiveSections] = useState({
//     basic: true, skills: true, education: true, projects: true, custom: true
//   });
...
//       {/* EDITOR SIDEBAR */}
//       <div className="w-[500px] h-full bg-white border-r flex flex-col shadow-xl">
//         <header className="p-6 border-b flex items-center justify-between sticky top-0 bg-white z-20">
//           <div className="flex items-center gap-4">
//             <h2 className="font-black text-slate-800 uppercase text-lg">ResuMake</h2>
//             <div className="flex gap-2 border-l pl-4">
//                <button onClick={() => setThemeColor('#2563eb')} className="w-5 h-5 rounded-full bg-blue-600" title="Blue" />
//                <button onClick={() => setThemeColor('#111827')} className="w-5 h-5 rounded-full bg-slate-900" title="Black" />
//                <button onClick={() => setThemeColor('#16a34a')} className="w-5 h-5 rounded-full bg-green-600" title="Green" />
//             </div>
//           </div>
//           <button onClick={downloadPDF} className="bg-slate-900 text-white px-3 py-1.5 rounded-lg font-bold text-sm">
//             {isDownloading ? <Loader2 className="animate-spin" size={14}/> : "DOWNLOAD PDF"}
//           </button>
//         </header>
...
//           {/* 1. Basic Info */}
//           <section>
//             <SectionHeader icon={User} title="1. Header Info" isOpen={activeSections.basic} onToggle={() => toggleSection('basic')} />
//             {activeSections.basic && (
//               <div className="p-4 border border-t-0 rounded-b-xl space-y-3 bg-slate-50/50">
//                 <InputField label="Name" value={form.full_name} onChange={e => setForm({...form, full_name: e.target.value})} />
...
//           {/* 2. Projects (FULLY DYNAMIC NOW) */}
//           <section>
//             <SectionHeader icon={Code} title="2. Projects" isOpen={activeSections.projects} onToggle={() => toggleSection('projects')} />
//             {activeSections.projects && (
//               <div className="p-4 border border-t-0 rounded-b-xl space-y-4 bg-slate-50/50">
//                 {form.projects.map((p, i) => (
//                   <div key={i} className="p-3 bg-white border rounded relative space-y-2">
//                     <button onClick={() => removeItem('projects', i)} className="absolute top-2 right-2 text-red-400"><Trash2 size={14}/></button>
//                     <InputField label="Project Name" value={p.title} onChange={e => handleArrayChange('projects', i, 'title', e.target.value)} />
//                     <InputField label="Tech Used" value={p.tech} onChange={e => handleArrayChange('projects', i, 'tech', e.target.value)} />
//                     <InputField label="Description" isTextArea value={p.desc} onChange={e => handleArrayChange('projects', i, 'desc', e.target.value)} />
//                   </div>
//                 ))}
//                 <button onClick={() => addItem('projects', {title:'', tech:'', desc:''})} className="w-full py-2 border-2 border-dashed text-blue-500 font-bold text-xs">+ ADD PROJECT</button>
//               </div>
//             )}
//           </section>
...
//           {/* 3. Skills */}
//           <section>
//             <SectionHeader icon={Briefcase} title="3. Skills" isOpen={activeSections.skills} onToggle={() => toggleSection('skills')} />
//             {activeSections.skills && (
//               <div className="p-4 border border-t-0 rounded-b-xl space-y-4 bg-slate-50/50">
//                 {form.skills.map((s, i) => (
//                   <div key={i} className="grid grid-cols-2 gap-2 p-2 bg-white border rounded relative">
//                     <InputField label="Category" value={s.category} onChange={e => handleArrayChange('skills', i, 'category', e.target.value)} />
//                     <InputField label="Skills" value={s.names} onChange={e => handleArrayChange('skills', i, 'names', e.target.value)} />
//                     <button onClick={() => removeItem('skills', i)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1"><Trash2 size={10}/></button>
//                   </div>
//                 ))}
//                 <button onClick={() => addItem('skills', {category:'', names:''})} className="text-blue-500 font-bold text-xs">+ ADD SKILL CATEGORY</button>
//               </div>
//             )}
//           </section>
...
//           {/* 4. Custom Sessions */}
//           <section>
//             <SectionHeader icon={Plus} title="4. Custom Sessions" isOpen={activeSections.custom} onToggle={() => toggleSection('custom')} />
//             {activeSections.custom && (
//               <div className="p-4 border border-t-0 rounded-b-xl space-y-4 bg-slate-50/50">
//                 {form.customSections.map((sec, sIdx) => (
//                   <div key={sIdx} className="p-3 bg-white border rounded relative space-y-3">
//                     <button onClick={() => removeItem('customSections', sIdx)} className="absolute top-2 right-2 text-red-500"><Trash2 size={14}/></button>
...
//                          <input key={iIdx} className="w-full text-xs p-2 bg-slate-50 border-b" value={item} onChange={e => handleCustomItemChange(sIdx, iIdx, e.target.value)} placeholder="Bullet point content..." />
//                        ))}
//                     </div>
//                     <button onClick={() => addCustomItem(sIdx)} className="text-blue-500 text-[10px] font-bold">+ ADD BULLET</button>
//                   </div>
//                 ))}
//                 <button onClick={() => setForm({...form, customSections: [...form.customSections, {title:'Awards', items:['']}]})} className="w-full py-2 border-2 border-dashed text-slate-500 font-bold">+ ADD NEW SESSION</button>
//               </div>
//             )}
//           </section>
//         </div>
...
//       {/* PREVIEW PANEL */}
//       <div className="flex-1 bg-slate-300 p-10 overflow-y-auto flex justify-center">
//         <div ref={resumeRef} className="w-[210mm] min-h-[297mm] bg-white shadow-2xl flex flex-col text-slate-800">
          
//           <header className="px-12 py-12 text-center border-b-[6px]" style={{ borderColor: themeColor }}>
//             <h1 className="text-5xl font-black uppercase tracking-tighter mb-4" style={{ color: themeColor }}>{form.full_name}</h1>
//             <div className="flex justify-center gap-6 text-[10pt] font-bold text-slate-500">
//               <span>{form.phone}</span> • <span>{form.email}</span>
//             </div>
//             <div className="mt-2 flex justify-center gap-4 text-blue-600 text-[9pt] font-black underline">
...
//             {/* MAIN (LEFT) */}
//             <div className="flex-[1.8] px-12 py-10 space-y-10">
//               <section>
//                 <h2 className="text-xs font-black uppercase tracking-[0.2em] mb-4" style={{ color: themeColor }}>Summary</h2>
//                 <p className="text-[10pt] leading-relaxed text-slate-600">{form.summary}</p>
//               </section>

//               <section>
//                 <h2 className="text-xs font-black uppercase tracking-[0.2em] mb-6" style={{ color: themeColor }}>Projects</h2>
//                 <div className="space-y-8">
//                   {form.projects.map((p, i) => (
//                     <div key={i}>
//                       <h3 className="font-black text-[11pt] text-slate-800 uppercase">{p.title}</h3>
//                       <p className="text-[9pt] font-bold italic mb-2" style={{ color: themeColor }}>{p.tech}</p>
//                       <p className="text-[10pt] text-slate-600">{p.desc}</p>
//                     </div>
//                   ))}
//                 </div>
//               </section>
...
//                 <section key={i}>
//                   <h2 className="text-xs font-black uppercase tracking-[0.2em] mb-4" style={{ color: themeColor }}>{sec.title}</h2>
//                   <ul className="text-[10pt] text-slate-600 space-y-2">
//                     {sec.items.map((item, j) => <li key={j} className="flex gap-2"><span>•</span> {item}</li>)}
//                   </ul>
//                 </section>
//               ))}
//             </div>
...
//             {/* SIDEBAR (RIGHT) */}
//             <div className="flex-1 bg-slate-50/50 px-8 py-10 border-l">
//               <section className="mb-12">
//                 <h2 className="text-xs font-black uppercase tracking-[0.2em] mb-6" style={{ color: themeColor }}>Skills</h2>
//                 <div className="space-y-4">
//                   {form.skills.map((s, i) => (
//                     <div key={i}>
//                       <p className="text-[8pt] font-black text-slate-400 uppercase mb-1">{s.category}</p>
//                       <p className="text-[10pt] font-bold text-slate-700">{s.names}</p>
//                     </div>
//                   ))}
//                 </div>
//               </section>
...
//               <section className="mb-12">
//                 <h2 className="text-xs font-black uppercase tracking-[0.2em] mb-6" style={{ color: themeColor }}>Education</h2>
//                 {form.educations.map((edu, i) => (
//                   <div key={i} className="mb-4">
//                     <p className="text-[10pt] font-black text-slate-800">{edu.degree}</p>
//                     <p className="text-[9pt] text-slate-500 uppercase">{edu.institution}</p>
//                   </div>
//                 ))}
//               </section>
